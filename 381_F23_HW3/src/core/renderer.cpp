...
void Renderer::setup_light_nodes()
{
	light_nodes_.reserve(NUM_LIGHTS);    
	for (int i = 0; i < NUM_LIGHTS; ++i)
	{
		sg::Node &light_node = create_light_node(LIGHT_POSITIONS[i]);
		light_nodes_.push_back(&light_node);    
	}
}


sg::Node &Renderer::create_light_node(const glm::vec3 &position)
{

	sg::Node &light_node = p_scene_->create_node();
	light_node.get_component<sg::Transform>().set_translation(position);
...
void Renderer::create_controller()
{
	...
	if (light_nodes_.size() == NUM_LIGHTS)
	{
		p_controller_ = std::make_unique<Controller>(*p_camera_node_,
	...
	{
		LOGE("Light nodes are not correctly initialized");
	}
}
